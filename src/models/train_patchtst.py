...
run_txt_log = f"logs/patchtst_train_{run_timestamp}.txt"
...
class PatchTST(nn.Module):
    def __init__(self, seq_len=200, patch_len=20, stride=10, d_model=64, n_heads=4, n_layers=3, dropout=0.2):
        super().__init__()
        self.patch_len = patch_len
        self.stride = stride
        
        # Calculate number of patches based on sequence length, patch length, and stride
        self.num_patches = int((seq_len - patch_len) / stride) + 1

        # Patch Projection (Linear mapping of each patch to d_model)
    ...
    def forward(self, x):
        # x shape: (Batch, Seq_Len) -> expand to (Batch, 1, Seq_Len)
        if x.dim() == 2:
            x = x.unsqueeze(1)

        # 1. Patching via unfold
        # Unfold extracts sliding local blocks from the sequence
        # Shape becomes: (Batch, 1, Num_Patches, Patch_Len)
        patches = x.unfold(dimension=-1, size=self.patch_len, step=self.stride)
        patches = patches.squeeze(1) # (Batch, Num_Patches, Patch_Len)

        # 2. Projection & Positional Encoding
        x_proj = self.patch_proj(patches) # (Batch, Num_Patches, d_model)
        x_proj = x_proj + self.pos_enc

        # 3. Transformer Attention
        attn_out = self.transformer(x_proj) # (Batch, Num_Patches, d_model)
...
def save_benchmark_logs(model_name, metrics):
    """Appends PatchTST metrics to master summary TXT and CSV files."""
...
def train():
    ...
    epochs = 40 # Slightly higher epochs for Transformer convergence
    start_time = time.time()

    logging.info("🚀 Training PatchTST Advanced Baseline...")
    ...
    logging.info("==== 🏆 FINAL PatchTST TEST RESULTS ====")
