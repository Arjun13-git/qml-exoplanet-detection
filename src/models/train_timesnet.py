...
run_txt_log = f"logs/timesnet_train_{run_timestamp}.txt"
...
# 3. TimesBlock (2D Inception Module for Periodicity)
class Inception2D(nn.Module):
    ...
    def forward(self, x):
        ...
        for k in self.kernels:
            res.append(k(x))
        return torch.mean(torch.stack(res, dim=0), dim=0)

class TimesBlock(nn.Module):
    def __init__(self, seq_len=200, top_k=3, d_model=32):
        super().__init__()
        self.seq_len = seq_len
        self.top_k = top_k
        self.conv = nn.Sequential(
            Inception2D(1, d_model),
            nn.GELU(),
            Inception2D(d_model, 1)
        )
    ...
    def forward(self, x):
        ...
        frequency_list = torch.abs(xf).mean(0)
        frequency_list[0] = 0  # Ignore DC component
        _, top_list = torch.topk(frequency_list, self.top_k)
        top_list = top_list.detach().cpu().numpy()

        period = [T // max(1, p) for p in top_list]

        res = []
        for i in range(self.top_k):
            p = period[i]
            # Padding if sequence length is not divisible by period p
            if T % p != 0:
                length = ((T // p) + 1) * p
                padding = torch.zeros(B, length - T, device=x.device)
                out = torch.cat([x, padding], dim=-1)
            else:
                length = T
                out = x

            # 2. Reshape 1D -> 2D tensor (Batch, 1, 2D_Height, 2D_Width)
            out = out.reshape(B, 1, length // p, p)
            
            # 3. 2D Convolution over Periodic Windows
            out = self.conv(out)
        ...
        # Aggregate representations across top-k periods
        res = torch.stack(res, dim=-1)
        # Softmax weighting based on FFT amplitudes
        amplitudes = torch.abs(xf)[:, top_list]
        weights = F.softmax(amplitudes, dim=-1).unsqueeze(1)
        
        res = torch.sum(res * weights, dim=-1)
        return res + x # Residual connection

class TimesNet(nn.Module):
    def __init__(self, seq_len=200, top_k=3, num_blocks=2, d_model=32):
        super().__init__()
        self.layer_norm = nn.LayerNorm(seq_len)
        self.blocks = nn.ModuleList([TimesBlock(seq_len, top_k, d_model) for _ in range(num_blocks)])
...
def save_benchmark_logs(model_name, metrics):
    """Appends TimesNet metrics to master summary TXT and CSV files."""
...
def train():
    ...
    logging.info("🚀 Training TimesNet 2D Periodicity Baseline...")
    ...
    logging.info("==== 🏆 FINAL TimesNet TEST RESULTS ====")
