...
# 3. iTransformer Architecture (Inverted Transformer)
class iTransformer(nn.Module):
    def __init__(self, seq_len=200, d_model=128, n_heads=4, n_layers=2, dropout=0.2):
        super().__init__()
        # Invert embedding: treat full time dimension as token embedding space
    ...
    def forward(self, x):
        ...
        # Inverted projection: map 1D sequence of length seq_len -> d_model representation
        x_emb = self.enc_embedding(x).unsqueeze(1) # (B, 1, d_model)
        x_emb = self.layer_norm(x_emb)
        
        # Attention across inverted token dimensions
        enc_out = self.transformer(x_emb) # (B, 1, d_model)
...
def train():
    ...
    logging.info("==== 🏆 FINAL iTransformer TEST RESULTS ====")
    ...
    save_benchmark_logs("iTransformer", {
