...
def load_xlstm_model(weights_path, device="cpu"):
    """
    Imports and instantiates the xLSTM classifier from src.models.train_xlstm or src.models.xlstm.
    """
    model_cls = None
    module_candidates = [
        ("src.models.train_xlstm", ["xLSTMClassifier", "XLSTMClassifier", "xLSTM", "XLSTM"]),
        ("src.models.xlstm", ["xLSTMClassifier", "XLSTMClassifier", "xLSTM", "XLSTM"])
    ]
    ...
    if model_cls is None:
        for mod_name in ["src.models.train_xlstm", "src.models.xlstm"]:
            try:
                ...
                for cls in classes:
                    if "Classifier" in cls.__name__ or "xLSTM" in cls.__name__:
                        model_cls = cls
    ...
    if model_cls is None:
        raise AttributeError("❌ No PyTorch nn.Module class found for xLSTM.")
...
def prepare_inputs_for_xlstm(inputs):
    """
    Ensures input shape matches xLSTM expected input dimensions (B, T) or (B, T, 1).
    """
    if inputs.ndim == 3 and inputs.shape[1] == 1:
        inputs = inputs.squeeze(1)  # Shape: (B, T)
    return inputs
...
def evaluate_xlstm(
    ...
    with torch.no_grad():
        for inputs, targets in test_loader:
            ...
            inputs = prepare_inputs_for_xlstm(inputs)
    ...
    plt.title(f"xLSTM CM (Thresh={best_threshold:.4f})")
    plt.xlabel("Predicted Label")
    plt.ylabel("True Label")
    plt.tight_layout()
    cm_path = os.path.join(output_fig_dir, "test_cm_xlstm.png")
    ...
    plt.title("xLSTM Test ROC Curve")
    plt.legend(loc="lower right")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    roc_path = os.path.join(output_fig_dir, "test_roc_xlstm.png")
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate xLSTM model on holdout test set.")
    parser.add_argument("--weights", type=str, default="models/saved/xlstm_weights.pth")
    ...
    try:
        ...
        model = load_xlstm_model(weights_path=args.weights, device=device)
        evaluate_xlstm(
            model=model, 
