...
def load_nimamba2_model(weights_path, device="cpu"):
    """
    Imports and instantiates NiMamba2Classifier dynamically.
    """
    model_cls = None
    module_candidates = [
        ("src.models.train_nimamba2", ["NiMamba2Classifier", "NiMamba2", "NIMamba2Classifier"]),
        ("src.models.nimamba2", ["NiMamba2Classifier", "NiMamba2", "NIMamba2Classifier"])
    ]
    ...
    if model_cls is None:
        for mod_name in ["src.models.train_nimamba2", "src.models.nimamba2"]:
            try:
                ...
                for cls in classes:
                    if "NiMamba" in cls.__name__ or "Mamba" in cls.__name__:
                        model_cls = cls
    ...
    if model_cls is None:
        raise AttributeError("❌ No PyTorch nn.Module class found for NiMamba-2.")
...
def evaluate_nimamba2(
    ...
    # Output Diagnostics
    print(f"\n==================== NiMamba-2 - PROBABILITY DIAGNOSTICS ====================")
    ...
    plt.title(f"NiMamba-2 CM (Thresh={best_threshold:.4f})")
    plt.xlabel("Predicted Label")
    plt.ylabel("True Label")
    plt.tight_layout()
    cm_path = os.path.join(output_fig_dir, "test_cm_nimamba2.png")
    ...
    plt.title("NiMamba-2 Test ROC Curve")
    plt.legend(loc="lower right")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    roc_path = os.path.join(output_fig_dir, "test_roc_nimamba2.png")
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate NiMamba-2 model on holdout test set.")
    parser.add_argument("--weights", type=str, default="models/saved/nimamba2_weights.pth")
    ...
    try:
        ...
        model = load_nimamba2_model(weights_path=args.weights, device=device)
        evaluate_nimamba2(
            model=model, 
